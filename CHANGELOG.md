...
### Changed
- **Policy: PyTorch weights only — no PaddlePaddle.** Reverted the experimental paddle
  `.pdparams` reader (protocol-4 pickle + inline numpy); the weight layer reads/writes
  PyTorch `.pth`/`.pt` exclusively. Paddle-origin checkpoints (e.g. RT-DETRv3) are out of
  scope. Documented as a hard rule in CLAUDE.md.
