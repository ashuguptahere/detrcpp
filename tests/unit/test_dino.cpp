...
//
// DINO smoke: multi-scale deformable encoder + query selection + deformable
// decoder forwards to the right shapes and trains with the focal path.
...
class DinoTest : public ::testing::Test {
 protected:
  void SetUp() override { RegisterBuiltins(); }
};

TEST_F(DinoTest, RegistersAndForwards) {
  EXPECT_TRUE(Registry::Instance().Contains("dino"));
  auto built = Registry::Instance().Build("dino", Tiny());
...
TEST_F(DinoTest, FocalTrainingStep) {
  auto model = *Registry::Instance().Build("dino", Tiny());
