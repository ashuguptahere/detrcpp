...
TEST(DinoV3StaParity, MatchesDeimv2LBackbone) {
  const std::string wpath = "/tmp/deimv2_l_backbone.safetensors";
  const std::string ppath = "/tmp/deimv2_l_bb/parity.safetensors";
  if (!std::filesystem::exists(wpath) || !std::filesystem::exists(ppath)) {
...
TEST(DinoV3StaParity, MatchesDeimv2XBackbone) {
  const std::string wpath = "/tmp/deimv2_x_backbone.safetensors";
  const std::string ppath = "/tmp/deimv2_x_bb/parity.safetensors";
  if (!std::filesystem::exists(wpath) || !std::filesystem::exists(ppath)) {
