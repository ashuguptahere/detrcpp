...
//
// DAB-DETR smoke: 4D anchor queries + HW-modulated attention + iterative anchor
// refinement forward to the right shapes and train with the focal path.
...
class DabDetrTest : public ::testing::Test {
 protected:
  void SetUp() override { RegisterBuiltins(); }
};

TEST_F(DabDetrTest, RegistersAndForwards) {
  EXPECT_TRUE(Registry::Instance().Contains("dab-detr"));
  auto built = Registry::Instance().Build("dab-detr", Tiny());
...
TEST_F(DabDetrTest, FocalTrainingStep) {
  auto model = *Registry::Instance().Build("dab-detr", Tiny());
