...
class DinoImpl : public IModel {
 public:
  ...
  Detections Forward(torch::Tensor images) override {
    ...
    auto enc_coord = enc_bbox_head_->forward(out_mem);  // unact deltas over the grid below

    auto anchors =
        InverseSigmoid(SelectAnchorCenters(shapes, images.options()).expand({b, -1, -1}));
    enc_coord = enc_coord + anchors;
 ...
 private:
  // Grid-center anchors (cx,cy,w,h) in [0,1]: [1, sum_hw, 4].
  torch::Tensor SelectAnchorCenters(const SpatialShapes& shapes, const torch::TensorOptions& opts) {
    std::vector<torch::Tensor> a;
    int lvl = 0;
    for (const auto& [h, w] : shapes) {
      auto grid = torch::meshgrid({torch::arange(h, opts), torch::arange(w, opts)}, "ij");
      auto xy = (torch::stack({grid[1], grid[0]}, -1).unsqueeze(0) + 0.5) /
                torch::tensor({static_cast<double>(w), static_cast<double>(h)}, opts);
      auto wh = torch::ones_like(xy) * (0.05 * std::pow(2.0, lvl));
      a.push_back(torch::cat({xy, wh}, -1).reshape({1, h * w, 4}));
      ++lvl;
    }
    return torch::cat(a, 1).clamp(1e-4, 1 - 1e-4);
  }
